...
export default function Promote() {
  ...
  const submit = async () => {
    if (!valid) { toast(t("p.check"), "err"); haptic("error"); return; }
    setBusy(true);
  ...
  const act = async (id: string, action: "pause" | "resume" | "delete") => {
    ...
    haptic("success");
    toast(action === "pause" ? t("p.paused") : action === "resume" ? t("p.resumed") : t("p.deleted"), "ok");
    refreshCore(); load();
  };
  ...
  return (
    <div className="px-4 pt-4 pb-2">
      <h1 className="font-display text-[19px] font-bold anim-rise">{t("p.title")}</h1>
      ...
      <div className="card mt-4 p-4 flex items-center gap-3 border-gold/30 anim-rise" style={{ animationDelay: "80ms" }}>
        ...
        <div className="grow text-[12.5px] text-mut leading-relaxed">
          <b className="text-ink">{t("p.pitch1")}</b>
          <span className="block text-dim">{t("p.pitch2")}</span>
        </div>
        <div className="text-right shrink-0">
          <div className="text-[10.5px] font-bold uppercase tracking-wider text-dim">{t("p.balLbl")}</div>
      ...
      <div className="card mt-4 p-4 anim-rise" style={{ animationDelay: "120ms" }}>
        <div className="text-[13px] font-extrabold uppercase tracking-wider text-mut mb-3">{t("p.newAd")}</div>
        <Field label={t("p.fTitle")}><input value={title} onChange={(e) => setTitle(e.target.value)} maxLength={60} placeholder="Summer sale — 50% off" className="input" /></Field>
        <Field label={t("p.fDesc")}><textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={2} maxLength={160} placeholder="One line that sells the click…" className="input resize-none" /></Field>
        <Field label={t("p.destUrl")}><input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://your-link.com" className="input tnum" /></Field>
        <Field label={t("p.fImage")} hint={t("p.imgHint")}><input value={image} onChange={(e) => setImage(e.target.value)} placeholder="https://…/banner.png" className="input" /></Field>

        <Field label={t("p.budgetLbl", { n: fmt(effBudget) })} hint={t("p.budgetHint", { a: fmt(minB), b: fmt(MAX_BUDGET) })}>
        ...
        <div className="card bg-panel/70 p-3.5 flex items-center justify-between">
          <div className="flex items-center gap-2 text-[13px] text-mut"><IcoEye size={16} className="text-sky" /> {t("p.reach")}</div>
          <div className="font-display text-[17px] font-bold tnum">{fmt(clicks)} <span className="text-[12px] text-mut font-body font-bold">{t("p.viewsW")}</span></div>
        </div>

        <Button full size="lg" className="mt-4" loading={busy} disabled={!valid} onClick={submit}>
          <IcoCoin size={18} /> {t("p.pubBtn", { n: fmt(effBudget) })}
        </Button>
        <div className="flex items-center justify-center gap-1.5 text-[12px] font-bold text-mint mt-2.5">
          <IcoBolt size={13} /> {t("p.instant")}
        </div>
      </div>

      {/* my campaigns */}
      <h2 className="font-display text-[13px] font-semibold uppercase tracking-[0.14em] text-mut mt-6 mb-1 anim-rise">{t("p.your")}</h2>
      <p className="text-[11.5px] text-dim mb-2.5">{t("p.yourSub")}</p>
      {mine.length === 0 ? (
        <Empty icon={<IcoLink size={20} />} title={t("p.none")} sub={t("p.noneSub2")} />
      ) : (
            ...
            <div key={c.id} className="stagger card p-4" style={{ "--i": i } as React.CSSProperties}>
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <div className="text-[11.5px] text-dim mt-0.5">{t("p.published", { x: timeAgo(c.created_at) })}</div>
                </div>
              ...
              <div className="mt-3">
                ...
                <div className="flex justify-between text-[11.5px] mt-1.5 tnum">
                  <span className="text-mut font-bold">{t("p.viewsOf", { a: fmt(c.clicks), b: fmt(c.max_clicks) })}</span>
                  <span className="text-dim">{t("p.spent", { a: fmt(c.spent), b: fmt(c.budget) })}</span>
                </div>
              </div>
      ...
      {/* edit budget */}
      <Modal open={!!editing} onClose={() => setEditing(null)} title={t("p.editBudget")} center>
        {editing && (
          <>
            <div className="text-[13px] font-bold truncate">{editing.title}</div>
            <div className="text-[11.5px] text-dim mt-0.5 mb-3 tnum">{t("p.curBudget", { a: fmt(editing.budget), b: fmt(editing.spent) })}</div>
            <Field label={t("p.newBudgetLbl", { n: fmt(editBudget) })} hint={`${fmt(editMin)} – ${fmt(MAX_BUDGET)}`}>
            ...
            <div className="card bg-panel/70 p-3.5 flex items-center justify-between">
              <span className="text-[12.5px] text-mut flex items-center gap-1.5"><IcoEye size={15} className="text-sky" /> {t("p.newReach")}</span>
              <span className="font-display text-[15px] font-bold tnum">{fmt(Math.floor(editBudget / settings.click_price))} <span className="text-[11px] text-mut font-body font-bold">{t("p.viewsW")}</span></span>
            </div>
            {editDelta !== 0 && (
              <div className={`text-[12px] font-bold mt-2.5 text-center ${editDelta > 0 ? "text-gold" : "text-mint"}`}>
                {editDelta > 0 ? t("p.willReserve", { n: fmt(editDelta) }) : t("p.willRefund", { n: fmt(-editDelta) })}
              </div>
            )}
            <Button full size="lg" className="mt-4" loading={editBusy} onClick={saveEdit}>{t("p.saveBudget")}</Button>
          </>
        )}
      </Modal>

      <div className="flex items-center justify-center gap-1.5 text-[11.5px] text-dim mt-6 mb-2">
        <IcoChipNote /> {t("p.foot")}
      </div>
    </div>
  );
}
